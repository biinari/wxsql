...
"""
Forward Local ports over SSH

Connects to an SSH server and sets up local port forwarding (like openssh -L
option) from a local port through a tunnelled connection to a destination
...
def main():
    ...
    if options.readpass:
        ...
        verbose('Now forwarding port %d to %s:%d ...' % (options.port,
            remote[0], remote[1]))
        ...
        try:
            ...
            print('Interrupted. Port forwarding stopped.')
            sys.exit(0)
